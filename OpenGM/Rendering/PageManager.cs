...
{
    public static Dictionary<string, (ImageMagick.MagickImage image, int id)> TexturePages = new();
    ...
    public static void UploadTexture(string name, ImageMagick.MagickImage image)
    {
        var newId = GL.GenTexture();

        GL.BindTexture(TextureTarget.Texture2D, newId);
        GL.TexImage2D(TextureTarget.Texture2D, 0, PixelInternalFormat.Rgba, (int)image.Width, (int)image.Height, 0, PixelFormat.Rgba, PixelType.UnsignedByte, image.GetPixels().ToArray());
