...
public static partial class VMExecutor
{
	...
	public static void PopToGlobalArray(string varName, int index, object? value)
	{
		VariableResolver.ArraySet(
			index,
			value,
			() => VariableResolver.GlobalVariables.TryGetValue(varName, out var array) ? array as List<object?> : null,
	...
	public static void PopToLocalArray(string varName, int index, object? value)
	{
		VariableResolver.ArraySet(
			index,
			value,
			() => Ctx.Locals.TryGetValue(varName, out var array) ? array as List<object?> : null,
	...
	public static void PopToSelfArray(GamemakerObject self, string varName, int index, object? value)
	{
		if (VariableResolver.BuiltInSelfVariables.TryGetValue(varName, out var gettersetter))
		{
			VariableResolver.ArraySet(
				index,
				value,
				() => gettersetter.getter(self) as List<object?>, // already did TryGetValue above
		...
		else
		{
			VariableResolver.ArraySet(
				index,
				value,
				() => self.SelfVariables.TryGetValue(varName, out var array) ? array as List<object?> : null,
